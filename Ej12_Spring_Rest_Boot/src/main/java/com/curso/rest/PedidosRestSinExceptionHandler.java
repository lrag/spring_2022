...
public class PedidosRestSinExceptionHandler {
	...
	@PutMapping(path = "/{id}",
			    ...
			    produces = MimeTypeUtils.APPLICATION_JSON_VALUE)	
	public ResponseEntity<?> modificar(@PathVariable("id") Integer idPedido,
...
/*
class Movida implements MethodInterceptor {
...
class Movida2 implements MethodInterceptor {
	...
	public Object invoke(MethodInvocation mi) throws Throwable {
		...
		ResponseEntity<Object> re = (ResponseEntity<Object>) retorno;
		if( re.getBody() == null) {
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}
